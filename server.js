...
// --- Utility funcs (იგივე ლოგიკა, რაც Make-ის JS-ში) ---
...
function replaceOneIndex(html, index, url) {
  ...
  // უჭერს მხარს src ან data-src, ორნიშნა ბრჭყალებს და ერთნიშნასაც
  ...
  // ფოლბექი მხოლოდ src-ზე
  const pattern2 = new RegExp(
    `(\\bsrc\\s*=\\s*)(["'])images\\/image${index}\\.${exts}\\2`,
    "i"
  );
  if (pattern2.test(html)) {
    html = html.replace(pattern2, (match, attr, quote) => `${attr}${quote}${url}${quote}`);
    return { html, replaced: true };
  }
 ...
 * B) { template_url, rows, urlFieldName?, idxFieldName? }  -> თვითონ ჩამოიტანს HTML-ს
 *
 * rows = [{ URL: "https://...", IDX: 1 }, ...]
 */
app.post("/replace-images", async (req, res) => {
  try {
    let { html, template_url, rows, urlFieldName, idxFieldName } = req.body || {};
    if (!html && !template_url)
      return res.status(400).json({ error: "Provide either 'html' or 'template_url'." });
